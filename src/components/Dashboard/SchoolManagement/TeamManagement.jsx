...
const TeamManagement = ({ schoolId }) => {
  ...
  const fetchManagers = async () => {
    ...
    try {
      const response = await fetch(`/api/schools/${schoolId}/managers`);
      if (response.ok) {
  ...
  const handleRemoveManager = async (managerId, managerName) => {
    try {
      const response = await fetch(`/api/schools/${schoolId}/managers/${managerId}`, {
        method: 'DELETE'
      });
  ...
  const handleChangeRole = async (managerId, newRole, managerName) => {
    try {
      const response = await fetch(`/api/schools/${schoolId}/managers/${managerId}/role`, {
        method: 'PATCH',
        headers: {
  ...
  // Show invite button even if no managers exist yet
  const showInviteButton = canInvite || !currentUserRole; // Allow invite if no role determined yet (backward compatibility)
          ...
          {managers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-6 max-w-md mx-auto">
                Vous êtes actuellement le seul gestionnaire de cette école. 
