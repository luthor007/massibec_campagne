...
import { Copy, Mail, Facebook, MessageSquare, CheckCircle, QrCode, Download } from 'lucide-react';
...
const ParentLetterModal = ({ isOpen, onClose, campaign, school }) => {
  ...
  // Generate join token and QR code when modal opens
  useEffect(() => {
    if (isOpen && campaign?._id) {
      generateJoinTokenAndQR();
    }
  }, [isOpen, campaign?._id]);

  const generateJoinTokenAndQR = async () => {
    if (!campaign?._id) return;

    setLoadingQR(true);
    try {
      // Generate join token
      const response = await fetch(`/api/campaigns/${campaign._id}/generate-join-token`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Generate QR code
        const qrDataUrl = await QRCodeLib.toDataURL(data.joinUrl, {
          width: 400,
          margin: 2,
          color: {
            dark: '#000000',
        ...
        console.error('Error generating join token');
...
En partageant votre boutique à vos proches, vous maximisez vos ventes et vos profits!`;
  ...
  const buildDonationSection = () => {
    ...
    if (studentDonationsEnabled && schoolDonationsEnabled) {
      // Both enabled
      section += `Les dons recueillis lors des ventes seront versés en partie à Jappuie, qui se chargera ensuite de remettre les montants destinés à ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization}.
Ainsi, vous conserverez dans votre compte une portion des dons ${terminology.participants} — soit ${studentCashPercentage} %, tandis que ${studentAccountPercentage} % sera transférée à Jappuie, puis reversée à ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization} afin d'être ajoutée au compte de votre enfant.
Quant aux dons que vous récolterez pour ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization}, vous devrez les transférer à 100 % à Jappuie, qui les reversera ensuite à ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization}.`;
...
Ainsi, vous conserverez dans votre compte une portion des dons ${terminology.participants} — soit ${studentCashPercentage} %, tandis que ${studentAccountPercentage} % sera transférée à Jappuie, puis reversée à ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization} afin d'être ajoutée au compte de votre enfant.`;
      ...
      section += `Les dons recueillis lors des ventes pour ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization} devront être transférés à 100 % à Jappuie, qui les reversera ensuite à ${terminology.organization === 'école' ? "l'" : "l'"}${terminology.organization}.`;
    }

    section += `
Le détail complet de ces répartitions, incluant les dons et les montants à remettre, sera clairement indiqué au moment où vous passerez votre commande à Jappuie, et pourra être suivi en tout temps dans l'onglet Statistiques de votre tableau de bord.`;
...
Exclusifs : les produits de qualité de votre fournisseur — disponibles uniquement via les campagnes de financement.
Populaires et faciles à vendre : les produits sélectionnés par votre fournisseur, reconnus et appréciés.
...
1. Inscription
https://jappuie.ca/
Cliquez sur « Je suis ${terminology.participant === 'étudiant' ? 'un élève' : 'un membre'} » ou scannez le QR code ci-dessous.
...
Une fois inscrit, entrez dans votre plateforme et ajoutez la campagne en scannant le QR code ci-dessous ou en entrant le code suivant :
...
Assurez-vous que toutes vos commandes soient marquées comme « Payé » avant de soumettre votre commande finale.

Commande finale
Date de commande : ${formatDateLong(campaign.endDate)} entre ${orderTimeRange}.
Cette étape est essentielle pour la vérification des commandes et des paiements par Jappuie.
Cliquez sur « Passer ma commande » une fois toutes vos ventes confirmées comme payées.
...
Les montants dus à Jappuie couvriront le coût des produits, ainsi que la part des profits${anyDonationsEnabled ? ' et des dons' : ''} qui seront déposés dans le compte ${terminology.participant}${schoolDonationsEnabled ? `, en plus des dons destinés à ${terminology.organization === 'école' ? "l'école" : "l'organisation"}` : ''}.
...
L'équipe Jappuie et ${terminology.organization === 'école' ? "l'école" : "l'organisation"} ${school.name}`;

  const facebookMessage = `🎉 Bonjour chers amis !
Je participe à la campagne de financement de ${terminology.organization === 'école' ? "l'école" : "l'organisation"} de mon enfant avec Jappuie.
Vous pouvez commander en ligne des produits de qualité pour soutenir notre campagne.
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white shadow-2xl border-2 border-gray-200 p-3 sm:p-6">
        ...
        <div className="space-y-4 sm:space-y-6 overflow-x-hidden">
          ...
          <Card className="border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
            <CardHeader className="p-3 sm:p-6">
              <CardTitle className="flex items-center space-x-2 text-base sm:text-lg text-blue-900">
                <QrCode className="h-5 w-5 sm:h-6 sm:w-6" />
                <span>QR Code pour rejoindre la campagne</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-3 sm:p-6">
              {loadingQR ? (
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
                  <div className="flex-shrink-0">
                    ...
                    <p className="text-xs text-center mt-2 text-gray-600">
                      Scannez pour rejoindre automatiquement
                    </p>
                  </div>
                  <div className="flex-1 space-y-3">
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">Lien de participation:</p>
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <Button
                          onClick={() => copyToClipboard(joinUrl, 'Lien QR')}
                          variant="outline"
                          size="sm"
                    ...
                    <Button
                      onClick={() => {
                        ...
                        link.download = `qr-code-campagne-${campaign.campaignCode || campaign._id}.png`;
                        link.href = qrCodeUrl;
                        link.click();
                    ...
                    <p className="text-xs text-gray-600">
                      Les parents peuvent scanner ce QR code pour rejoindre automatiquement la campagne lors de leur inscription.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="text-center py-4">
                  <p className="text-gray-600">Erreur lors de la génération du QR code</p>
                  <Button
                    onClick={generateJoinTokenAndQR}
                    variant="outline"
                    className="mt-2"
                  >
                    Réessayer
