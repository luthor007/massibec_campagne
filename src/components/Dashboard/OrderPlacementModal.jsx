...
import { calculateMultipleOf6Suggestion, getNextMultipleOf6, aggregateProductsWithSuggestions } from '@/utils/inventoryHelpers';
...
export default function OrderPlacementModal({
    ...
    // Aggregate products from paid orders and calculate suggestions
    const productBreakdown = useMemo(() => {
        if (!paidOrders || paidOrders.length === 0) return {};
        return aggregateProductsWithSuggestions(paidOrders);
    }, [paidOrders]);
    ...
    const calculatedTotals = useMemo(() => {
        ...
        // but also must be paid to Massibec. The equation is:
        ...
        return {
            ...
            // Amount to pay to Massibec (cost + school donations + school account donations)
    ...
    // One-click function to make all products multiples of 6
    const handleMakeAllMultiplesOf6 = () => {
        ...
        toast.success('Tous les produits ont été ajustés aux multiples de 6! 🎉');
    };
    ...
    const handleAdditionalQuantityChange = (productName, delta) => {
        setAdditionalProducts(prev => {
            ...
            let newValue;
            if (delta > 0) {
                // Add: always add 6 units (or go to next multiple if not at one)
                if (currentTotal % 6 === 0) {
                    // Already at a multiple, add 6 more
                    newValue = current + 6;
                } else {
                    // Not at a multiple, go to next multiple
                    const nextMultiple = getNextMultipleOf6(currentTotal);
                    const needed = nextMultiple - currentTotal;
                    newValue = current + needed;
                }
            } else {
                // Subtract: go to previous multiple of 6 (or subtract 6 if already at multiple)
                if (currentTotal % 6 === 0 && currentTotal >= 6) {
                    // Already at a multiple, subtract 6
                    newValue = Math.max(0, current - 6);
                } else {
                    // Not at a multiple, go to previous multiple
                    const previousMultiple = Math.floor(currentTotal / 6) * 6;
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            ...
            if (!isOrderingPeriod) {
                toast.error('Vous ne pouvez passer votre commande que le jour de fin de campagne entre minuit et midi.');
                setLoading(false);
                return;
            }
            ...
            // Check if all products are multiples of 6 (combine sold + additional quantities)
            ...
            const invalidProducts = Array.from(productQuantities.entries())
                .filter(([_, totalQty]) => totalQty % 6 !== 0)
            ...
            // All products are multiples of 6, proceed with order
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <DialogTitle className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                                🎉 Félicitations! Passez votre commande à Massibec
                            </DialogTitle>
                ...
                <div className="space-y-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="mt-4 p-4 bg-white rounded-lg border-2 border-green-300">
                                            <p className="text-sm text-gray-700">
                                                ...
                                                Partagez votre boutique avec votre famille et vos amis pour maximiser vos ventes! En bonus, à partir du moment où vous passez votre commande à Massibec, vous gardez 100% de tout les dons recueillis.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {/* One-Click Multiples of 6 Button */}
                    {!allProductsAreMultiplesOf6 && (
                        <motion.div
                        ...
                        >
                            <Button
                                onClick={handleMakeAllMultiplesOf6}
                                variant="default"
                                size="lg"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span>Ajuster tous les produits aux multiples de 6 (un clic!)</span>
                                </motion.span>
                                {isOrderingPeriod && (
                                    <motion.div
                        ...
                        <Card className={!isOrderingPeriod ? 'opacity-60 pointer-events-none' : ''}>
                            ...
                            <CardContent>
                                <div className="mb-4 p-4 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg">
                                    <p className="text-sm text-gray-700">
                                        <strong>💡 Pourquoi des multiples de 6 ?</strong>
                                    </p>
                                    <p className="text-sm text-gray-600 mt-2">
                                        ...
                                        Les produits arrivent de l&apos;usine en boîtes de 6, et commander en multiples de 6 permet d&apos;éviter de déballer et réemballer les produits,
                                        ce qui accélère la préparation de votre commande et réduit les risques d&apos;erreurs.
                                    </p>
                                    <p className="text-sm text-gray-600 mt-2">
                                        <strong>Exemples :</strong> 6 tartes aux fraises, 12 tartes au sucre, 6 pâtés à la viande
                                    </p>
                                </div>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                <TableHead className="text-right">Multiple de 6?</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {productBreakdownArray.map((product) => {
                                                ...
                                                const nextMultiple = totalQty > 0 && totalQty % 6 === 0
                                                    ? totalQty + 6  // If already at multiple, next is +6
                                                    : getNextMultipleOf6(totalQty); // Otherwise, get next multiple
                                                const stillNeeded = nextMultiple - totalQty;
                                                // Dynamic suggestion - always show next multiple (minimum 6)
                                                const dynamicSuggestion = stillNeeded > 0 ? stillNeeded : 6;
                    ...
                    {financialBreakdown.length > 0 && (
                        <Card className="border-2 border-blue-200">
                            <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50">
                                ...
                                <p className="text-sm text-gray-600 mt-2">
                                    Voici exactement ce que vous payez à Massibec et où va votre argent
                                </p>
                            </CardHeader>
                            <CardContent>
                                    ...
                                    <Card className="bg-red-50 border-red-200">
                                        <CardContent className="pt-4">
                                            ...
                                            <div className="text-xs text-red-600 mt-2 space-y-1">
                                                <div>
                                                    <span className="font-semibold">{calculatedTotals.totalCost.toFixed(2)}$</span> Coût produit (à Massibec)
                                                </div>
                                ...
                                <Alert className="mt-6 bg-gradient-to-r from-green-50 to-emerald-50 border-green-300">
                                    ...
                                    <AlertDescription className="text-sm text-gray-700">
                                        ...
                                        <p className="mt-2">
                                            Une fois que vous avez passé votre commande à Massibec, vous gardez <strong>100% de tous les dons</strong> que vous recevez.
                                ...
                                {!allProductsAreMultiplesOf6 && (
                                    <Alert className="bg-orange-50 border-orange-200">
                                        ...
                                        <AlertDescription className="text-sm">
                                            Certains produits ne sont pas en multiples de 6. Utilisez le bouton ci-dessus pour ajuster automatiquement.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            </div>
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    ...
                                    Confirmer la commande à Massibec 🎉
                                </>
                            )}
                        </Button>
                    </div>
            ...
            <Dialog open={showNonMultipleConfirmation} onOpenChange={setShowNonMultipleConfirmation}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Certains produits ne sont pas en multiples de 6
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                            <p className="text-sm text-gray-700 mb-3">
                                <strong>Les produits suivants ne sont pas en multiples de 6 :</strong>
                            </p>
                            <ul className="list-disc list-inside space-y-1 text-sm text-gray-600">
                                {pendingOrderData?.invalidProducts.map(({ productName, totalQty }) => (
                                    <li key={productName}>
                                        {productName}: {totalQty} unités (multiple de 6 recommandé: {Math.ceil(totalQty / 6) * 6})
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <Alert className="bg-red-50 border-red-200">
                            ...
                            <AlertDescription className="text-sm text-gray-700">
                                <strong className="text-red-700">Important :</strong> Si vous confirmez cette commande sans ajuster pour des boîtes de 6,
                        ...
                        <p className="text-sm text-gray-600">
                            Êtes-vous sûr de vouloir passer votre commande sans ajuster pour des boîtes de 6 ?
                        </p>
                    </div>
