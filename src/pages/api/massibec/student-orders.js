...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use cursor to handle corrupted documents gracefully
    // The error occurs when MongoDB tries to deserialize documents with invalid UTF-8
    let orders = [];
    let corruptedCount = 0;

    try {
      // First, try the normal approach
      orders = await Order.find(query).sort({ createdAt: -1 }).limit(10000).lean();
    } catch (error) {
      // If we get a BSON UTF-8 error, use raw MongoDB driver to skip corrupted documents
      if (error.message && (error.message.includes('UTF-8') || error.message.includes('Invalid') || error.message.includes('BSON') || error.name === 'BSONError')) {
        console.error('[massibec/student-orders] BSON UTF-8 error detected, using raw MongoDB driver to skip corrupted documents...');
        ...
        // Convert query to MongoDB format (handle ObjectIds)
        const mongoQuery = {};
        for (const [key, value] of Object.entries(query)) {
          if (key === '$or') {
            mongoQuery.$or = value;
          } else if (key === 'createdAt' && value && typeof value === 'object') {
        ...
        try {
          // Use toArray with error handling - but this might still fail
          // So we'll use a manual iteration with try-catch around each next()
          const cursor = collection.find(mongoQuery).sort({ createdAt: -1 }).limit(10000);
          ...
          while (hasMore && consecutiveErrors < maxConsecutiveErrors) {
            try {
              // Check if there are more documents - this might throw if next document is corrupted
              try {
                hasMore = await cursor.hasNext();
              } catch (hasNextError) {
                // If hasNext() throws, the next document is likely corrupted
                // Try to skip it by calling next() and catching that error
                corruptedCount++;
                consecutiveErrors++;
                console.error(`[massibec/student-orders] Error in hasNext(), attempting to skip corrupted document:`, hasNextError.message);

                try {
                  // Try to get the corrupted document to skip it
                  await cursor.next();
                } catch {
                  // If next() also fails, we can't continue
                  hasMore = false;
                }
              ...
              // Get next document - this is where the BSON deserialization happens
              ...
              // Try to convert the document, handling potential UTF-8 issues
              // Use a replacer function to sanitize strings
              const sanitized = JSON.parse(JSON.stringify(doc, (key, val) => {
                if (typeof val === 'string') {
                  ...
                  try {
                    ...
                    try {
                      ...
                      // Last resort: remove non-printable characters
              ...
              corruptedCount++;
              consecutiveErrors++;
              // Try to get the document ID for logging
              let docId = 'unknown';
              try {
                // The error might have occurred before we got the doc
                // Try to get it from the error or cursor state
                if (docError._id) {
                  docId = docError._id.toString();
                }
              } catch {
                // Can't get ID
              }
              console.error(`[massibec/student-orders] Skipping corrupted order ${docId} (${consecutiveErrors}/${maxConsecutiveErrors} consecutive errors):`, docError.message);

              // If we hit too many consecutive errors, stop trying
              if (consecutiveErrors >= maxConsecutiveErrors) {
                console.error(`[massibec/student-orders] Too many consecutive errors (${consecutiveErrors}), stopping cursor iteration`);
                hasMore = false;
                break;
              }
            }
          ...
          console.error('[massibec/student-orders] Error with cursor:', cursorError);
          // If we got some orders before the error, return them
        ...
        // Re-throw if it's not a UTF-8 error
        throw error;
      }
    }

    if (corruptedCount > 0) {
      console.warn(`[massibec/student-orders] Skipped ${corruptedCount} corrupted order(s) due to invalid UTF-8 data`);
    }
