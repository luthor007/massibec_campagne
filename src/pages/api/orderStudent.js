  ...
  if (req.method === 'GET') {
    try {
      ...
      if (startDate || endDate) {
        ...
        if (endDate) {
          ...
          if (dateFilter.$gte && parsedEnd < dateFilter.$gte) {
    ...
    try {
      ...
      products.forEach(product => {
        const { productName, quantity, price, cost, isAdditional } = product;
      ...
      try {
        ...
        if (userId && campaignId) {
          ...
          const soldOrders = await Order.find({
            ...
            status: 'Payé' // Only count paid orders that haven't been ordered to Massibec yet
          }).lean();
